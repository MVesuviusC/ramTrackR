...
read_monitor_data <- function(monitor) {
  if (!is(monitor, "RamMonitor")) stop("monitor must be a RamMonitor object")
...
monitor_bg_routine <- function(
  ...
  while (running) {
    tryCatch({
      ...
      cat(as.character(Sys.time()), ": ", e$message, "\n", file = log_path, append = TRUE)
      running <<- FALSE
    })
...
start_monitoring <- function(
  ...
  if (is.null(pid) && is.null(process_name)) {
    ...
    if (nrow(matches) == 0) stop("Process not found")
    pid <- matches$pid[1]
  }
  ...
  reg.finalizer(env_px, function(e) {
    if (e$px$is_alive()) {
      e$px$kill()
    }
  }, onexit = TRUE)
...
#' @export
setGeneric("stop_monitoring", function(monitor, cleanup = FALSE) standardGeneric("stop_monitoring"))
...
#' @export
setGeneric("bookmark_time", function(monitor, label) standardGeneric("bookmark_time"))

#' @rdname bookmark_time
#' @export
setMethod("bookmark_time", signature(monitor = "RamMonitor", label = "character"), function(monitor, label) {
...
#' @export
setGeneric("get_current_ram_usage", function(monitor) standardGeneric("get_current_ram_usage"))
...
setMethod("get_current_ram_usage", "RamMonitor", function(monitor) {
  ...
  if (nrow(ram_data) == 0) return(NA_real_)
...
#' @export
setGeneric("get_max_ram_usage", function(monitor) standardGeneric("get_max_ram_usage"))
...
if (getRversion() >= "2.15.1") {
  utils::globalVariables(c("timestamp", "memory_mb", "time", "memory_bytes", "stage", "label"))
}
